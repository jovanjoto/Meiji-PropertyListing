...
class Views(db.Model):
    ...
    @classmethod
    def incrementViews(cls, propertyId:str, month:int, year:int) -> bool:
        views = Views.query.filter_by(propertyListingId=propertyId, month=month, year=year).one_or_none()
        # if views for a property at a certain time doesn't exist, create new views record
        if not views:
            newViews = cls(propertyListingId=propertyId, month=month, year=year, views=1)
