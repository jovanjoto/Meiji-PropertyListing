...
export default function CreateNewPropertyModal({ state, setState }) {
	...
	const enterDetails = (details) => {
		if (
			...
			details.num_bedrooms > 0 &&
			details.num_bedrooms <= 8 &&
			details.area > 0 &&
			details.price > 0 &&
			details.name.trim() != "" &&
			details.address.trim() != "" &&
			details.seller_email.trim() != "" &&
			...
			formData.append("num_of_bedrooms", details.num_bedrooms);
			formData.append("num_of_bathrooms", details.num_bathrooms);
			formData.append("district", details.district);
			formData.append("type", details.property_type);
			formData.append("area", details.area);
			formData.append("seller_email", details.seller_email);
