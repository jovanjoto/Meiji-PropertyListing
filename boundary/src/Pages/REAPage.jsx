	...
	const searchFilter = (keyword, filter) => {
		...
		agent.forEach((agentJson) => {
			...
			let search_bool3 = agentJson.first_name
				...
				.startsWith(keyword.toLowerCase());
			if (
				(search_bool || search_bool2 || search_bool3) &&
				agentJson.avg_rating >= filter.minRate &&
				agentJson.avg_rating <= filter.maxRate
			) {
				filtered_list.push(agentJson);
			}
		});
	...
	return (
		<div className="mx-10 my-6">
			...
			<div className="mt-8">
				...
				{!loading &&
					(search == "" && filter.minRate == 0 && filter.maxRate == 5
						? agent.length > 0
							? displayList(agent)
							: displayEmptyList()
